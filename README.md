...
Each keyword rule supports this format:

`keyword, color, [flash], [timer], [border width], [opacity], [group name]`

If `[group name]` is set, tabs in the same browser window that match that rule are automatically placed into a Chrome tab group with that name and the rule color.
